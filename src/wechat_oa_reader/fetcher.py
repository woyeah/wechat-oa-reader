...
class Fetcher:
    def __init__(self, proxy_pool: ProxyPool | None = None, rate_limiter: RateLimiter | None = None):
    ...
    async def fetch_article(
        ...
        try:
            ...
            html = await self.fetch_page(full_url, extra_headers=headers, timeout=timeout)
    ...
    async def _fetch_curl_cffi(self, url: str, headers: dict[str, str], timeout: int, proxy: str | None) -> str:
        ...
        return await loop.run_in_executor(self._executor, self._fetch_curl_cffi_sync, url, headers, timeout, proxy)

    @staticmethod
    def _fetch_curl_cffi_sync(url: str, headers: dict[str, str], timeout: int, proxy: str | None) -> str:
        kwargs: dict[str, Any] = {"timeout": timeout, "allow_redirects": True, "verify": False}
        if proxy:
            kwargs["proxy"] = proxy
